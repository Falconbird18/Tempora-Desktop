...
export default function Notification(props: NotificationsProps) {
  ...
  const ActionsBox = () =>
    notification.get_actions() && (
      <box className="notification__actions">
        {notification.get_actions().map((action: any) => (
          <button
            ...
            on_clicked={() => {
              try {
                ...
                console.error(
                  `Error invoking action for notification ${action.id}: ${e}`,
                );
              }
            }}
            hexpand={true}
  ...
  const revealer = new Widget.Revealer({
    ...
    setup: (self: any) => {
      ...
      if (idleResult && typeof idleResult.catch === "function") {
        idleResult.catch((error: any) => {
      ...
      try {
        timeoutId = timeout(transitionDuration, () => {
          ...
          if (!revealer || !revealer["get_parent"]()) {
            ...
            if (callback) callback(); // Call callback even if already disposed
            return;
          }
          ...
          if (_removeWidgetFromParent) {
            ...
            if (revealer["get_parent"]())
              revealer["get_parent"]().remove(revealer);
            revealer.destroy();
          }
          if (callback) callback(); // Call callback after removal and destruction
        });
      } catch (error) {
