  ...
  return (
    ...
    >
      ...
      <box horizontal spacing={spacing}>
        <button onClicked={() => changePlayer(-1)}>
...
const PlayerSwitcher = ({
  ...
  selectedPlayer: Variable<string>;
}) => {
    ...
    <revealer revealChild={players.as((p) => p.length > 0)}>
      <overlay>
        <box
          ...
          setup={(self) => {
            ...
            self.hook(mpris, "notify::players", () => {
              ...
              if (
                selectedPlayer.get() === "" ||
                !ps.some((p) => p.busName === selectedPlayer.get())
              ) {
        ...
        >
          <eventbox
            onScroll={(self, event) =>
              changePlayer(event.direction === Gdk.ScrollDirection.UP ? 1 : -1)
            }
          >
            <stack
            ...
            >
              {players.as((ps) =>
                ps.map((player) => (
                  <Player player={player} changePlayer={changePlayer} />
                )),
              )}
            </stack>
          </eventbox>
          <revealer
            ...
            revealChild={players.as((p) => p.length > 1)}
          >
            <box
              valign={Gtk.Align.CENTER}
              halign={Gtk.Align.CENTER}
                ...
                ps.map((player, idx) => (
                  <box
                    ...
                    setup={(self) => {
                      ...
                      self.toggleClassName(
                        "selected",
                        selectedPlayer.get() === player.busName,
                      );
                      self.hook(selectedPlayer, (_, selected) => {
                        self.toggleClassName(
                          "selected",
                          selected === player.busName,
                        );
                      });
                    }}
                  ></box>
