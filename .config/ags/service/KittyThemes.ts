...
export class KittyThemesService {
    private static generateConfig(theme: string, mode: string, colors: { background: string, primary: string, secondary: string, text: string }) {
        return `# vim:ft=kitty
...
color15 #${colors.text}`
    ...
    private static async getThemeColors(theme: string, mode: string): Promise<{ background: string, primary: string, secondary: string, text: string }> {
        ...
        try {
            ...
            console.log(`Theme colors for ${theme}${mode}:`, colors);
            return colors;
        } catch (error) {
            ...
            const themePromises = themes.flatMap(theme =>
                modes.map(async mode => {
                    try {
                        ...
                        console.log(`Generated Kitty theme: ${theme}${mode}`);
                    } catch (error) {
