...
const isNetworkError = (out: any) => {
    return !out || out === null || out === undefined || typeof out === 'string' && out.includes('curl: (6)') // curl error 6 is "Couldn't resolve host"
};
...
export const barWeather = Variable<any | null>(null).poll(
  ...
  (out, prev) => {
      try {
          ...
          const outputWithoutIcon = out.replace(/[^\x00-\x7F↖↗↙↘↔←→↑↓]/g, '').trim();
          const trimmedOutput = outputWithoutIcon.replace(/\s+/g, ' ');
          const cleanedOutput = trimmedOutput.replace(/\+(\d+)([°]?)([CF])/, '$1°$3');
